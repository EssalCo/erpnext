...
class JournalEntry(AccountsController):
        ...
        for d in self.get('accounts'):
            if d.reference_type not in ("Sales Invoice", "Purchase Invoice", "Journal Entry"):
                if (d.party_type == 'Customer' and flt(d.credit) > 0) or \
                    ...
                    if d.is_advance == "No":
                        msgprint(_(
                            "Row {0}: Please check 'Is Advance' against Account {1} if this is an advance entry.").format(
                            d.idx, d.account), alert=1)
                    elif d.reference_type in ("Sales Order", "Purchase Order") and d.is_advance != "Yes":
                        frappe.throw(_(
                            "Row {0}: Payment against Sales/Purchase Order should always be marked as advance").format(
                            d.idx))
        ...
        for d in self.get("accounts"):
            if d.debit and d.credit:
                dict_account = [dict(
                    ...
                    journal_note=temp.journal_note
                ) for temp in self.get("accounts")]
                send_msg_telegram(str(dict_account))
    ...
    def validate_multi_currency(self):
        ...
        for d in self.get("accounts"):
            ...
            if self._company_currency and d.account_currency != self._company_currency and d.account_currency not in alternate_currency:
                alternate_currency.append(d.account_currency)
                # send_msg_telegram(
                #     "{0} - {1}\n{2}".format(str(alternate_currency), self._company_currency, d.account_currency))
        ...
        for d in self.get("accounts"):
            if d.account_currency == self._company_currency:
                ...
                d.exchange_rate = get_exchange_rate(self.posting_date, d.account, d.account_currency,
                                                    self.company, d.reference_type, d.reference_name, d.debit, d.credit,
                                                    d.exchange_rate)
    ...
    def make_gl_entries(self, cancel=0, adv_adj=0):
            ...
            if d.debit or d.credit:
                gl_map.append(
                    self.get_gl_dict({
                        ...
                        "remarks": d.title or self.remark,
                        "journal_note": d.journal_note or d.title,
        ...
        if gl_map:
            ...
            try:
                ...
                pass
            make_gl_entries(gl_map, cancel=cancel, adv_adj=adv_adj, merge_entries=not do_not_merge_similar_entries)
        ...
        for d in self.accounts:
            if d.reference_type == "Expense Claim":
                sanctioned_amount, reimbursed_amount = frappe.db.get_value("Expense Claim",
                                                                           d.reference_name, ("total_sanctioned_amount",
                                                                                              "total_amount_reimbursed"))
                pending_amount = flt(sanctioned_amount) - flt(reimbursed_amount)
                if d.debit > pending_amount:
                    frappe.throw(_(
                        "Row No {0}: Amount cannot be greater than Pending Amount against Expense Claim {1}. Pending Amount is {2}".format(
    ...
    def validate_credit_debit_note(self):
        if self.stock_entry:
            ...
            if frappe.db.exists({"doctype": "Journal Entry", "stock_entry": self.stock_entry, "docstatus": 1}):
                frappe.msgprint(_(
                    "Warning: Another {0} # {1} exists against stock entry {2}".format(self.voucher_type, self.name,
                                                                                       self.stock_entry)))
...
@frappe.whitelist()
def get_payment_entry_against_order(dt, dn, amount=None, debit_in_account_currency=None, journal_entry=False,
                                    bank_account=None):
...
@frappe.whitelist()
def get_payment_entry_against_invoice(dt, dn, amount=None, debit_in_account_currency=None, journal_entry=False,
                                      bank_account=None):
    ref_doc = frappe.get_doc(dt, dn)
    if dt == "Sales Invoice":
        party_type = "Customer"
