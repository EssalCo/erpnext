...
cur_frm.cscript.custom_cheque_date = function() {
	if(!cur_frm.doc.cheque_date){
		return
	}
	frappe.call({
				method: "erpnext.utilities.hijri_date.convert_to_hijri",
				args:{
					date:cur_frm.doc.cheque_date
				},
				callback: function (r) {
					if (r.message) {
						cur_frm.set_value("cheque_hijri_date", r.message);
...
cur_frm.cscript.custom_bill_date = function() {
	if(!cur_frm.doc.bill_date){
		return
	}
	frappe.call({
				method: "erpnext.utilities.hijri_date.convert_to_hijri",
				args:{
					date:cur_frm.doc.bill_date
				},
				callback: function (r) {
					if (r.message) {
						cur_frm.set_value("bill_hijri_date", r.message);
