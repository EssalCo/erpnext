...
def execute():
    try:
        ...
        show_unclosed_fy_pl_balances = data.get('show_unclosed_fy_pl_balances')
        if show_unclosed_fy_pl_balances:
            filters['show_unclosed_fy_pl_balances'] = show_unclosed_fy_pl_balances
...
def prepare_data(accounts, filters, total_row, parent_children_map, company_currency):
    ...
    for d in accounts:
        has_value = False
        if d.indent == 0:
            name = '<span style="color:#131D5A;">{0}</span>'.format(('{} - {}'.format(d.account_number, d.account_name)
            ...
            name = '<span style="color:#5A1327;">{0}</span>'.format(('{} - {}'.format(
                d.account_number, d.account_name)
                                                                     if d.account_number else d.account_name))
        elif d.indent == 4:
            name = '<span style="color:#25592A;">{0}</span>'.format(('{} - {}'.format(d.account_number, d.account_name)
                                                                     if d.account_number else d.account_name))
        else:
            name = '<span style="color:#664406;">{0}</span>'.format(('{} - {}'.format(d.account_number, d.account_name)
                                                                     if d.account_number else d.account_name))
