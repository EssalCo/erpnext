...
def set_account_currency(filters):
    if filters.get("account") or (filters.get("party") and len(filters.party) == 1):
        filters["company_currency"] = frappe.get_value(
...
def get_gl_entries(filters):
    ...
    gl_entries = frappe.db.sql(
        """
		select jv.posting_date, jv.name as voucher_no, jv.voucher_type, jvd.account, jvd.debit_in_account_currency as debit, 
			jvd.credit_in_account_currency as credit, jvd.cost_center, jvd.against_account, 
			jvd.journal_note, is_opening {select_fields}
		from
			`tabJournal Entry Account` jvd, `tabJournal Entry` jv
		where jvd.parent = jv.name and jv.docstatus=1 and company=%(company)s {conditions}
