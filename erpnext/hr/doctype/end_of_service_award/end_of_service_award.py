...
class EndofServiceAward(Document):
    ...
    def on_submit(self):
        ...
        if len(expenses) > 0:
            ...
            msg = """Expense cliam has been created: <b><a href="#Form/Expense Claim/{0}">{0}</a></b>""".format(ec_doc.name)
    ...
    def get_leave_balance(self,employee):
        ...
        if total_leave_balance:
            leave_days = frappe.db.sql("select sum(total_leave_days) from `tabLeave Application` where employee='{0}' and docstatus =1 and posting_date between '{1}' and '{2}'".format(employee,total_leave_balance[0][1],total_leave_balance[0][2]))[0][0]
            if not leave_days:
                leave_days = 0
