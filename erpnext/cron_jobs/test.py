...
def execute():
   ...
   round(sum(`tabGL Entry`.credit_in_account_currency), 4) as  credit_in_account_currency
            from `tabGL Entry`
            where `tabGL Entry`.company="أعمال النماء" and `tabGL Entry`.party_type="Supplier"  and party="مؤسسة سالم باطيب لمواد البناء"  group by `tabGL Entry`.voucher_type, `tabGL Entry`.voucher_no, `tabGL Entry`.account, `tabGL Entry`.cost_center 
