...
def execute_again():
    ...
    journal_entry = frappe.get_doc(
        dict(
            ...
            title="القيد الإفتتاحي لسنة ٢٠٢٠",
            voucher_type="Journal Entry",
            naming_series="JV-",
            posting_date="2020-01-01",
            company=company.name,
            user_remark="القيد الإفتتاحي لسنة ٢٠٢٠",
            multi_currency=0,
            remark="القيد الإفتتاحي لسنة ٢٠٢٠٢٠١٩",
            bill_date="2019-12-31",
            third_party_creation="2020-01-01",
            accounts=[],
            is_opening="Yes",
        )
    )
    ...
    with open(current_file, 'rb') as csvfile:
        ...
        for row in spamreader:
            if row[0] == "Account" or not row[0]:
                continue
            debit = float(re.sub(r'\s', '', row[1].replace(" ", "").replace("-", "").replace("\\t", "").rstrip()) or 0)
            credit = float(re.sub(r'\s', '', row[2].replace(" ",  "").replace("-", "").replace("\\t", "").rstrip()) or 0)
            print debit
            print credit
