...
<br>الرجاء تجديد جواز سفرك وتحديث بياناتك عند الموارد البشرية<br><br>نتمنى لك يوما سعيدا<br>
        ...
        if employee.health_inssurance_valid_to and employee.alarm_inssurance_expiry and datetime.strptime(
            ...
            alert = frappe.get_doc(
                ...
                notify_on_every_login=0,
                expire_notification_on=(datetime.now() + timedelta(days=1)).date(),
                content="""<div align="right"><b><font color="#397B21">مرحبا بك</font><br>نود ثذكيرك بأن <font color="#9C0000">تاريخ تأمينك الصحي على وشك الإنتهاء</font><font color="#9C0000"> {} </font><br>الرجاء تجديد تأمينك الصحي وتحديث بياناتك عند الموارد البشرية<br><br>نتمنى لك يوما سعيدا<br></b></div>""".format(
                    ".. باقٍ على تاريخ إنتهائه أقل من ({}) أسبوع".format(remained_days / 7) if remained_days > 0 else "انتهى تاريخ صلاحية تأمينك الصحي"
                ),
            )
            alert.insert(ignore_permissions=True)
